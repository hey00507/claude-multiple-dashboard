...
hook 스크립트와 동일하게 config.json에서 포트를 읽는다:
```bash
CONFIG_FILE="$HOME/.claude-dashboard/config.json"
if [ -f "$CONFIG_FILE" ] && command -v jq &>/dev/null; then
  DASH_PORT=$(jq -r '.port // 7420' "$CONFIG_FILE")
else
  DASH_PORT=7420
fi
```
...
```bash
CWD=$(pwd)
curl -s -X DELETE "http://localhost:${DASH_PORT}/api/session-defaults" \
...
**B. 대시보드 PATCH API로 세션 업데이트 (SSE broadcast 포함):**
API를 통해 업데이트하면 서버가 SSE로 브라우저에 실시간 전파한다. API 실패 시 파일 직접 수정으로 fallback:
```bash
# PATCH API 호출용 JSON body 구성 (name만, color만, 또는 둘 다)
PATCH_BODY=$(python3 -c "
import json
body = {}
name = '이름'  # 파싱된 이름 (없으면 빈 문자열)
color = '색상'  # 파싱된 색상 (없으면 빈 문자열)
if name: body['projectName'] = name
if color: body['color'] = color
print(json.dumps(body))
")

API_OK=$(curl -s -o /dev/null -w '%{http_code}' -X PATCH "http://localhost:${DASH_PORT}/api/sessions/${SESSION_ID}" \
  -H "Content-Type: application/json" \
  -d "$PATCH_BODY" --connect-timeout 1 --max-time 2 2>/dev/null)

if [ "$API_OK" != "200" ]; then
  # Fallback: 파일 직접 수정
  SESSION_FILE="$HOME/.claude-dashboard/sessions/${SESSION_ID}.json"
  if [ -f "$SESSION_FILE" ]; then
    PARSED_NAME="이름" PARSED_COLOR="색상" SESSION_FILE="$SESSION_FILE" python3 -c "
import json, os
...
```bash
CWD=$(pwd)
curl -s -X PUT "http://localhost:${DASH_PORT}/api/session-defaults" \
  -H "Content-Type: application/json" \
