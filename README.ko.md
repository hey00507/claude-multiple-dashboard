...
Claude Code를 3~5개 동시에 띄워 작업하면, 모든 터미널이 똑같이 생겼다. 어느 탭이 어떤 프로젝트인지 하나하나 들어가서 확인해야 한다.

![전부 같아 보이는 세션들](docs/screenshots/problem-no-labels.png)

## 해결

이 대시보드는 모든 세션을 한 화면에 모아서, 실시간 상태 + idle time + 세션 색상 + 활동 히스토리를 보여준다.

![세션 색상이 적용된 대시보드](docs/screenshots/session-colors.png)

`/session-setting`으로 이름을 붙이면 터미널 statusline과 대시보드 양쪽에 색상이 표시된다:
...
![이름이 붙은 터미널 세션들](docs/screenshots/terminal-sessions.png)
...
| **세션 프리셋** | 프로젝트 디렉토리별 이름/색상 기본값 저장, 다음 세션에 자동 적용 |
...
세션에 이름과 색상을 부여하면 터미널 statusline과 대시보드 양쪽에 반영됩니다.
...
**프로젝트 기본값**은 `~/.claude-dashboard/config.json`에 저장되며, 같은 디렉토리에서 새 세션 시작 시 자동 적용됩니다. `~/`같은 범용 경로는 매칭에서 제외됩니다.
...
- 세션 이름은 컨텍스트 압축 시에도 **3-tier fallback**으로 유지 (/tmp → 세션 JSON → config 기본값)
