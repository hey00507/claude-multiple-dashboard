...
- [ ] **히스토리 내보내기** — 현재 필터 기준으로 JSON/CSV 다운로드 버튼
- [ ] **세션 + 로그 통합 삭제** — 세션 삭제 시 관련 로그도 함께 제거, 비활성 세션 일괄 삭제 버튼
  - DELETE /api/sessions/:id 에 로그 삭제 로직 추가
  - "종료된 세션 모두 삭제" 버튼 (inactive sessions 영역)
- [ ] **프로젝트 디렉토리별 세션 그룹핑** — 같은 cwd의 세션을 그룹으로 묶어서 표시
  - 그룹 헤더에 프로젝트 경로 + 세션 수
  - 그룹 접기/펼치기
  - 그룹 내 정렬은 기존 상태 우선순위 유지

### 4-2. 테마 & 키보드 확장
- [ ] **다크/라이트 테마 토글** — 시스템 설정 연동 + 수동 토글 버튼, localStorage 저장
- [ ] **Enter 키 활용** — 선택된 세션에서 Enter → 세션 상세 풀뷰 전환 (사이드패널 → 전체화면)

### 4-3. 통계 & 코드 구조
- [ ] **세션 통계 대시보드** — 일별 프롬프트 수, 도구 사용 빈도 Top 10, 평균 idle time 등
  - GET /api/stats?date= API 추가
  - 통계 탭 또는 별도 페이지
- [ ] **app.js 모듈 분리** — 751줄 단일 파일 → ES Module 분리
  - sessions.js (세션 카드 렌더링, 그룹핑)
  - history.js (히스토리 필터, 검색, 페이지네이션)
  - detail.js (상세 패널, 타임라인)
  - utils.js (공통 유틸: formatDuration, htmlEscape, truncate)
  - sse.js (SSE 연결, 이벤트 핸들링)
